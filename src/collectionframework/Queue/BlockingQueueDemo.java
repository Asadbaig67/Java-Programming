...
class Producer implements Runnable {
    ...
    @Override // This thread can act as a class that brings data from database in batches and stores data in the same array. which is being used by processor.
    public void run() { // Multithreading logic is here
        while (true) {
            try {
                System.out.println("Producer produced: " + value);
                queue.put(value++);
                Thread.sleep(1000); // here this wait time could act as function that fetches data from db.
            } catch (Exception e) {
    ...
    public void run() {
        while (true) {
            try {
                ...
                Thread.sleep(2000); // It could be an API that takes time in processing
            } catch (Exception e) {
                Thread.currentThread().interrupt();
